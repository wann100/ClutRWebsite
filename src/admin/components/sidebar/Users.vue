<template lang="html">
    <div class="users__container">
        <h2 style="color:white"class="listings_header">Users</h2>
        <div class="ui feed" v-for="user in users"
            :key="user.userDetails.uid"
...
export default {
    ...
    methods: {
        ...
        changeChannel(user) {
            ...
            const channel = { id: channelId, name: user.userDetails.email }
        ...
        getNotification(user) {
            const channelId = this.getChannelId(user.userDetails.uid)
            let notif = 0
        ...
        resetNotification(user = null) {
            let channelId = null
            if (user !== null) {
                channelId = this.getChannelId(user.userDetails.uid)
            } else {
